...
(function ($, Drupal, once) {
  ...
  Drupal.behaviors.formModal = {
    attach: function (context, settings) {
      ...
      if (!settings.datatables || !settings.datatables.tableId) {
        console.log('datatables settings not available yet');
        return;
      }
      ...
      // Try to get the DataTable instance - use once() on the actual table or a wrapper
      // For AJAX-loaded content, we need to look for the table within the context
      const $table = $('#' + tableId, context);

      if ($table.length > 0) {
        once('datatable-instance-' + tableId, '#' + tableId, context).forEach(function() {
      ...
      function setupDataTableEvents() {
        ...
        console.log(drupalSettings.datatables);
      ...
      function enableRowSelection() {
        ...
        $(document).on('click', '#' + tableId + ' tbody tr td', function (e) {
          ...
          if (columnIndex >= drupalSettings.datatables.colIdIdx && $(e.target).closest('.btn').length === 0) {
      ...
      function addRefreshButton() {
        ...
        const tableId = drupalSettings.datatables.tableId;

        // DataTables puts search inside the *_wrapper
        const filterDiv = $('#' + tableId + '_wrapper').find('div.dt-search');

        console.log('CEK FILTER DIV', filterDiv);
